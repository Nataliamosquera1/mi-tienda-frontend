...
export class ProductosComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // Captura la subcategoría desde la URL (?subcategoria=ID)
  ...
  cargarProductos(subcategoriaId?: number) {
    ...
    let productos = [
      ...
      { id: 4, nombre: 'Loción', precio: 68900, imagen: 'locion.jpg', subcategoriaId: 10 },
    ...
    // Filtrar por subcategoría si viene en la URL
    if (subcategoriaId) {
      productos = productos.filter(p => p.subcategoriaId === subcategoriaId);
    }

    // Mantener imágenes desde carpeta PUBLIC (raíz del servidor)
  ...
  deleteProduct(id: number) {
    if (confirm('¿Eliminar producto?')) alert('Eliminar ID: ' + id);
  }
